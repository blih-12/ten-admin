            ...
            <div className="overflow-x-auto">
              <table className="w-full min-w-[720px]">
                <thead className="bg-gray-900">
                  <tr>
                    {['Title', 'Teams / Competitions', 'Status', 'Date', 'Actions'].map(h => (
                ...
                <tbody>
                  {articles.map(a => (
                    <tr key={a._id} className="border-t border-gray-700 hover:bg-gray-750">
                      <td className="px-5 py-3 max-w-xs">
                        ...
                        <div className="text-gray-500 text-xs mt-0.5 line-clamp-1">{truncate(a.excerpt, 70)}</div>
                      </td>
                      <td className="px-5 py-3 text-gray-400 text-xs max-w-[160px]">
                        <div className="line-clamp-1">{a.teams?.map(t => t.name).join(', ') || a.competitions?.map(c => c.name).join(', ') || '—'}</div>
                      </td>
                      <td className="px-5 py-3"><Badge status={a.status} /></td>
                      <td className="px-5 py-3 text-gray-500 text-xs whitespace-nowrap">{formatDate(a.createdAt)}</td>
